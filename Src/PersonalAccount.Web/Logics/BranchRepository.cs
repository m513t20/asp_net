...
public class BranchRepository(PersonalAccountContext context, ISettingsRepository settingsRepostory) : IBranchRepository
{
    ...
    public BranchModel GetBranch(Guid id)
    {
        ...
        if (result is null) throw new InvalidOperationException($"Невозмодно найти филиал по коду {id}!");

        var settings = _settingsRepostory.Load(result);
        result.Settings = settings!;
        return result;
    }

    /// <inhericdoc/
    public async Task<BranchModel> GetBranchAsync(Guid id, CancellationToken token)
        => await Task.Run(() => GetBranch(id), token);
